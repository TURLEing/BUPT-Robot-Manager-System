...
<head>
  ...
  List<exploreDto> explores = (List<exploreDto>)session.getAttribute("explores");
  %>
  <title> Querying for Exploring Information - BUPT Robot Manager System </title>
  ...
  <div id="layoutSidenav_content">
    <main>
      <div class="container-fluid px-4">
        <h1 class="mt-4">Querying for Exploring Information</h1>
        <h4 class="breadcrumb mb-4">
          <li class="breadcrumb-item active"> <%="Totally "+ explores.size() +" Exploration Records"%>  </li>
        </h4>
        <div class="card mb-4">
          ...
          <div class="card-body">
            <table id="datatablesSimple">
              ...
              <%
                int index = 0;
                for ( exploreDto explore: explores ) {
              %>
              <tr>
                <td><%=explore.getExplorationId()%></td>
                <td><%=explore.getMazeId()+"-th Maze"%></td>
                <%if (explore.getTreasureId() <= 0) {%>
                  <td>Treasure Not Found!</td>
                <% } else {%>
                  <td><%=explore.getTreasureId()+"-th Treasure"%></td>
                <% } %>
                <td><%=explore.getStartTime()%></td>
                <td><%=explore.getEndTime()%></td>
              </tr>
              <% index = index+1; }%>
              </tbody>
            </table>
